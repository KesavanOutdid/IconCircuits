...
interface ServiceConfig {
  ...
  supplier_type?: ConfigOption;
  warranty_period?: ConfigOption;
  quality_assurance?: ConfigOption;
}
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  components: "e.g., 10,50,100",
  lead_time_days: "e.g., 5,10,15",
  supplier_type: "e.g., Authorized,Distributor,Direct",
  warranty_period: "e.g., 6M,1Y,2Y",
  quality_assurance: "e.g., Standard,Premium,Certified",
};

const REQUIRED_FIELDS = ["components", "lead_time_days", "supplier_type", "warranty_period", "quality_assurance"];

export default function EditComponentSourcing() {
    ...
    <>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-body-1xlg font-bold text-dark dark:text-white">
          EDIT COMPONENT SOURCING SERVICE
        </h1>
        <button
      ...
      <div className="rounded-[10px] bg-white px-7.5 pb-4 pt-7.5 shadow-1 dark:bg-gray-dark dark:shadow-card">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-4">
            <button
              type="submit"
              disabled={!isFormValid() || loading || !hasDataChanged()}
              className="rounded bg-primary px-8 py-2.5 text-sm font-medium text-white hover:bg-opacity-90 disabled:bg-gray-300 dark:disabled:bg-gray-600"
            >
            ...
            <button
              type="button"
              onClick={() => router.back()}
              className="rounded border border-[#E8E8E8] px-8 py-2.5 text-sm font-medium text-dark hover:bg-gray-100 dark:border-form-strokedark dark:text-white dark:hover:bg-gray-700"
            >
              Cancel
            </button>
          </div>
